 ...
 * cette méthode pourrait fonctionner. Cette méthode ne fonctionne pas.
 */
...
ordreIdempotence(M, p) = {
   ...
   * on pourrait aussi élever la matrice à la puissance chaque diviseur de l'ordre de ce groupe, bien que cela soit très lent.
   */
  R = Mod(M, p);
  k = 1;
  id = Mod(matid(12), p);
